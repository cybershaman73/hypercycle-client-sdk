...
def check_health(client, slot):
    """
    Attempt /health. Returns one of three outcomes:
      "ready"      — health endpoint exists and model_ready == true
      "not_ready"  — health endpoint exists but model not yet ready
      "no_endpoint" — AIM has no /health endpoint (404)
      "error"      — connection or unexpected error
    """
    ...
    model_ready = health.data.get("model_ready", True)  # absent = assume ready
    if model_ready:
...
def wait_for_health(client, slot, max_wait_sec=300, poll_interval=15):
    """
    Poll /health until model_ready or until we determine there is no
    /health endpoint. Returns ("ready"|"no_endpoint"|"timeout"), last_data.
    """
    print(f"Checking /health (up to {max_wait_sec//60}min)...")
    elapsed = 0
    while elapsed < max_wait_sec:
        ...
        if outcome == "no_endpoint":
            print(f"  [{elapsed:3d}s] /health → 404 (no health endpoint on this AIM)")
            return "no_endpoint", None

        if outcome == "not_ready":
            print(f"  [{elapsed:3d}s] /health → model_ready=False — waiting...")
        else:
            print(f"  [{elapsed:3d}s] /health → error: {data}")
